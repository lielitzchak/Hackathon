...
module.exports = {
    register: async (req, res) => {
        try {
            ...
            let { firstName, lastName, profileImg, email,password, image, title , jobs } = req.body;
            bcrypt.hash(password, 10, async (err, hashPassword) => {
                if (err) return res.status(400).json({ success: false, message: err.message });
                password = hashPassword;
                const user = new User({firstName, lastName, profileImg, email,password, image, title , jobs })
    ...
    login: async (req, res) => {
        try {
            ...
            bcrypt.compare(req.body.password, user.password, (err, isMatch) => {
                ...
                const token = jwt.sign({ user }, process.env.SEACRET_KEY, { expiresIn: "3h" });
